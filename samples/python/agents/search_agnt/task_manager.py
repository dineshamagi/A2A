...
class AgentTaskManager(InMemoryTaskManager):
    def __init__(self, agent: SearchAgent, notification_sender_auth: PushNotificationSenderAuth):
        super().__init__()
        self.agent = agent
    ...
    async def _run_streaming_agent(self, request: SendTaskStreamingRequest):
        ...
        try:
            ...
            async for item in self.agent.stream(query, session_id):
                logger.info("***********************************************************")
                logger.info(f"INSIDE STREAM: query={query}, sessionId={session_id}")
                logger.info("**********************************************************")
    ...
    def _validate_request(
        ...
        if not utils.are_modalities_compatible(
            task_send_params.acceptedOutputModes, SearchAgent.SUPPORTED_CONTENT_TYPES
        ):
            logger.warning(
                "Unsupported output mode. Received %s, Support %s",
                task_send_params.acceptedOutputModes,
                SearchAgent.SUPPORTED_CONTENT_TYPES,
            )
